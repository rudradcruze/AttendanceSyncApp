...
function loadSynchronizations(page) {
    ...
    $.get(APP.baseUrl + 'Attandance/GetMyRequests', {
        ...
        renderPagination('#pagination', pagedData.TotalRecords, pagedData.Page, pagedData.PageSize, loadSynchronizations);
    }).fail(function (xhr) {
        if (xhr.status === 401) {
            window.location.href = APP.baseUrl + 'Auth/Login';
        } else {
...
/* ============================
   Pagination (Using Common.js)
============================ */
// renderPagination is now in Common.js
...
/* ============================
   Helpers (Using Common.js functions)
============================ */
// formatDate, getStatusText, getStatusClass, escapeHtml are now in Common.js
