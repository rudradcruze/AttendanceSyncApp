...
$(function () {
    ...
    // Tab change event - load data when switching tabs
...
function loadCompanyRequests(page) {
    ...
    $.get(APP.baseUrl + 'AdminRequests/GetAllCompanyRequests', {
        page: page,
...
function openCompanyStatusModal(requestId) {
    ...
    // Load request info
    $.get(APP.baseUrl + 'AdminRequests/GetCompanyRequest', { id: requestId }, function (res) {
        if (res.Data) {
            var req = res.Data;
...
function updateCompanyStatus() {
    ...
    $.ajax({
        url: APP.baseUrl + 'AdminRequests/UpdateCompanyRequestStatus',
        type: 'POST',
        data: {
            requestId: requestId,
            status: status
